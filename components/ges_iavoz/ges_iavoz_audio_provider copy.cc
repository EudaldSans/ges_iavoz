...
void IAVoz_AudioProvider_I2STask ( void * vParam ) {
    ...
    uint32_t i2s_read_buffer[i2s_bytes_to_read / 2] = {};
    ...
    for ( ;; ) {
        ...
        if (bytes_read <= 0) {
            ...
            int16_t *buffer_s16 = (int16_t *) i2s_read_buffer;
            int sample = 1;
            for (; sample < bytes_read / sizeof(uint32_t); sample += 2) {
                buffer_s16[sample/2] =  (int16_t) (i2s_read_buffer[sample] >> 15);
            }

            printf("Reached sample %d\n", sample);

            /* write bytes read by i2s into ring buffer */
            int bytes_written = rb_write(ap->audio_capture_buffer, (uint8_t*)buffer_s16, bytes_read / 4, 10);
            ...
            * arrived */
            ...
            ESP_LOGD(TAG, "%d-%d-%d-%d", buffer_s16[0], buffer_s16[1], buffer_s16[2], buffer_s16[3]);

            if (bytes_written <= 0) {ESP_LOGE(TAG, "Could Not Write in Ring Buffer: %d ", bytes_written);} 
            else if (bytes_written < bytes_read / 4) {ESP_LOGW(TAG, "Partial Write");}
        }
    }
}
