...
esp_event_loop_handle_t events_conn_loop_h;
...
void events_conn_handler (void* handler_arg, esp_event_base_t base, int32_t id, void* event_data) {
    ...
    switch(id){
        ...
        case EVENT_CONN_WEIGHTS_TRANSMISSION: {
            //ESP_LOGI(TAG, "WEIGHTS SENT");
            uint8_t payload[3 + kCategoryCount];
            payload[0] = EVENT_CONN_TRANSMISSION;
            payload[1] = 0;
            payload[2] = 3 + kCategoryCount;
            memcpy(&payload[3], event_data, kCategoryCount);

            esp_event_post_to(events_conn_loop_h, EVENTS_CONN, EVENT_CONN_TRANSMISSION, payload, sizeof(payload), portMAX_DELAY);

            //send_tcp(payload, 3 + kCategoryCount);
        } break;

        case EVENT_CONN_TRANSMISSION: {
            ESP_LOGI(TAG, "WEIGHTS RECEIVED");
        } break;
